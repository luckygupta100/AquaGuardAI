...
GROQ_API_KEY = "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"   # ← Yahan apni key daalo
...
PREDICT_EVERY  = 10   # Har 10 seconds mein predict kare
...
def get_groq_prediction(sensors, flags, score):
    ...
    prompt = f"""You are an expert epidemiologist for AquaGuard — a rural India water monitoring system.

Live sensor readings:
- TDS: {sensors['tds']:.1f} mg/L  (WHO limit: <500)
- Turbidity: {sensors['turbidity']:.1f} NTU  (WHO limit: <4)
- Temperature: {sensors['temperature']:.1f}°C  (Safe: 10-30)
- Salinity: {sensors['salinity']:.2f} ppt  (WHO limit: <0.5)
- pH: {sensors['ph']:.1f}  (WHO range: 6.5-8.5)

Detected flags:
{flags_text}

Pre-analysis contamination score: {score:.0f}/100

Based on WHO outbreak data and waterborne disease patterns in rural Rajasthan, India, give disease risk percentages.
...
{{
  ...
  "typhoid_risk": <0-100>,
  "diarrhea_risk": <0-100>,
  "dysentery_risk": <0-100>,
  "overall_contamination": <0-100>,
  "drinkable": <true or false>,
  "severity": "<safe|low|medium|high|critical>",
  "primary_concern": "<one sentence about biggest risk>",
...
def upload(sensors, pred):
    ...
    # Latest prediction
    r1 = requests.put(PREDICT_URL, json=payload, timeout=5)
    print(f"✅ Firebase upload: {r1.status_code}")

    # History
    requests.post(HISTORY_URL, json=payload, timeout=5)
